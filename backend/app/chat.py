    ...
    try:
        ...
        if not request.awaiting_followup:
            ...
            if needs_followup:
                ...
                if user_id and not user_id.startswith("anonymous_"):
                    chat_doc = await chats_collection.find_one({"_id": safe_object_id(chat_id)})
                    if chat_doc:
        ...
        chat = await chats_collection.find_one({"_id": safe_object_id(chat_id)})
        messages = chat.get("messages", [])
    ...
    async def generate_sse() -> AsyncGenerator[str, None]:
        try:
            ...
            if not is_valid:
                ...
                # Preserve awaiting_followup state so user can still answer the pending follow-up
                yield f"data: {json.dumps({'type': 'done', 'awaiting_followup': chat_request.awaiting_followup})}\n\n"
            ...
            # Fetch current chat document (needed for original query + conversation history)
            chat_doc = await chats_collection.find_one({"_id": safe_object_id(chat_id)})
            existing_msgs = chat_doc.get("messages", []) if chat_doc else []
            ...
            # Build conversation history for LLM context (last 4 turns = 8 messages)
            conversation_history = [
                {"role": m["role"], "content": m["content"]}
                for m in existing_msgs[-8:]
