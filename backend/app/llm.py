...
class LLMService:
    ...
    async def assess_query_specificity(self, question: str) -> bool:
        ...
        """
        try:
            prompt = f"""Classify this patient query. Does it need a follow-up clarifying question before answering?

Query: "{question}"

RULE: If the patient is describing a personal symptom, answer NEEDED unless they already provided ALL THREE of: duration, severity, AND context/associated symptoms.

NEEDED — patient symptom without full detail:
- "I have eye pain" → NEEDED
- "i have small eye pain" → NEEDED
- "my stomach hurts" → NEEDED
- "I have a headache" → NEEDED
- "chest pain" → NEEDED
- "I feel tired" → NEEDED
- "my knee hurts for 2 days" → NEEDED (no severity or context)

NOT_NEEDED — information request (not a personal symptom):
- "What is diabetes?" → NOT_NEEDED
- "Side effects of aspirin" → NOT_NEEDED
- "How to lower cholesterol?" → NOT_NEEDED
- "What is an MRI?" → NOT_NEEDED
- "Tell me about asthma" → NOT_NEEDED

NOT_NEEDED — symptom WITH complete detail (duration + severity + context all present):
- "Sharp chest pain radiating to my left arm, started 30 min ago, severity 8/10" → NOT_NEEDED
...
Please provide a comprehensive response following the format specified."""
            ...
            # Build messages: system + prior turns (for continuity) + current RAG message
            messages_to_send = [{"role": "system", "content": system_prompt}]
            if conversation_history:
                messages_to_send.extend(conversation_history[-8:])  # last 4 turns
...
The conversation history (prior turns) is provided. Use it to maintain context across the full conversation.
- If the user's message is a conversational update (e.g., "I am fine now", "Thank you", "Got it", "Feeling better"), respond warmly as a caring doctor would — acknowledge their update, offer a brief follow-up note or wellness reminder, and do NOT force the structured clinical format for such messages.
- If the user is clearly following up on a prior complaint, factor in everything discussed so far.
- Never repeat advice already given in the same conversation unless the user asks.

MANDATORY CLOSING (always include for medical responses):
---
