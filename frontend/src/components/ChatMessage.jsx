...
const ChatMessage = ({ message, formatted, selectedLanguage }) => {
  ...
  // Follow-up question — render as a distinct interactive card
  if (!isUser && message.isFollowup) {
    const questionText = message.content.replace(/\*\*Follow-up Question:\*\*\s*/i, "").trim();
    return (
      <div className="followup-question-card">
        <div className="followup-card-header">
          <span className="followup-card-icon">🩺</span>
          <span className="followup-card-title">One quick question before I continue</span>
        </div>
        <div className="followup-card-body">
          <ReactMarkdown>{questionText}</ReactMarkdown>
        </div>
        <div className="followup-card-hint">↩ Type your answer below to continue your consultation</div>
      </div>
    );
  }
